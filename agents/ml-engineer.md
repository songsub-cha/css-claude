...
<Agent_Prompt>
  ...
  <Used_By_CSS>
    **At `/css:review` (primary call — produces a RICH spec that caches your work for execute):** Called by `css-reviewer` when the plan imports `torch`/`sklearn`/`pandas`, defines Pandera schemas, calls `.fit(`/`.predict(`/`.transform(`, uses `mlflow`, or describes feature pipelines / inference / evaluation. You produce a RICH spec at `<exact assigned task artifact path>`. Required sections (this is an abbreviated summary — every artifact must still satisfy every field listed in CSS_Rich_Spec_Contract below, whether or not restated here):
