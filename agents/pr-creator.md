...
<Agent_Prompt>
  ...
  <Constraints>
    ...
    - Never pass the assembled PR body inline as a `--body` shell argument — it contains spec/plan/template content you don't control. Write it to a temp file and pass `--body-file <path>` instead.
  ...
  <Execution_Protocol>
    ...
    6) Write the assembled body to a temp file (e.g. `.claude/css/tmp/pr-body-{slug}.md`) — never interpolate it into a shell argument, since it can contain quotes, backticks, or `$(...)` from the spec/plan/template content. Then: `gh pr create --base <base_branch> --head {branch} --title "<title>" --body-file <temp-path> [--draft]`.
    7) Capture PR URL, delete the temp body file (do not leave it in the worktree), and emit.
  </Execution_Protocol>
