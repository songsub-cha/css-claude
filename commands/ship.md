...
8. **Stage 4 — execute**: invoke `/css:execute --slug <slug>`. The `master_flow` flag tells `/css:execute` not to ask Gate 2 again (it inherits the answer from this step).
...
12. **Stage 7 — pr**: invoke `/css:pr --slug <slug>` (with `--draft` if user chose). The `master_flow` flag tells `/css:pr` not to ask Gate 3 again.
