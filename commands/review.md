...
2. Enforce the two-attempt review counter, acquire the review lock, and update `_active.json` (`latest_slug`, `active_epic`, `active_phase`).
...
9. On PASS record `phases.review.status`, `verdict`, `level`, `artifact`, exact executable `rich_specs`, and separate `advisories`. On loopback, update retry counters and invoke the required earlier stage. Release the lock.
