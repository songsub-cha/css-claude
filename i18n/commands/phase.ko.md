...
10. `locks/{child_slug}-phasing.lock` 을 사용한다(60분 경과 시 stale → 안내와 함께 교체; 다른 실행의 신선한 락 → 안내와 함께 중단); `_active.json.active_epic` 과 `active_phase` 를 갱신한다; 모든 종료 경로에서 락을 해제한다.
