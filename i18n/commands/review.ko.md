...
7. advisory 는 별도로 디스패치한다; 이 리뷰어들은 쓸 수 없으므로 반환된 각 리포트를 `.claude/css/reviews/` 아래 저장한다:
   ...
   advisory 경로는 실행 가능한 Rich Spec 이 아니다. 아키텍처든 보안이든 어느 advisory 에서라도 CRITICAL/HIGH 발견 사항(본문 스캔이 아니라 해당 advisory 의 `VERDICT=ISSUES_FOUND critical=<n> high=<n> ...` 마지막 줄에서 읽음)이 있으면 `LOOPBACK_TO_PLAN` 을 유발한다.
