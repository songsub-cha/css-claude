...
아이디어를 입력하면 spec 작성 → 계획 → 검토 → TDD 구현 → 검증 → 문서화 → PR까지 자동으로 진행되고, PR 머지 후 정리는 `/css:clean`이 맡습니다. 총 22개의 전문 에이전트가 단계별로 투입되며, 고비용 결정 시점에는 사람의 승인 게이트가 개입합니다.

```
/css:interview  →  /css:plan  →  /css:phase  →  /css:review  →  /css:execute  →  /css:verify  →  /css:document  →  /css:pr
                                                                                                                        ↑
                                                         /css:ship  ──── 3개 승인 게이트로 전체 실행 ────────────────────┘
                                                                                       (PR 머지 후 정리: /css:clean)
```
...
단계별: `$css-interview`, `$css-plan`, `$css-phase`, `$css-review`, `$css-execute`, `$css-verify`, `$css-document`, `$css-pr`. PR 머지 후 정리: `$css-clean`. 프로젝트 문서 큐레이션: `$css-wiki`.
...
- **Epic → Phase 서브이슈**: `/css:phase`가 큰 아이디어를 여러 Phase로 분리하면, 각 Phase는 Epic 이슈 아래에 **GitHub 네이티브 서브이슈**(내장 중첩 목록 + 진행률 표시)로 등록되어 자체 이슈로 내용이 개별 동기화됩니다. 서브이슈 API가 없는 구버전 GitHub에서는 Epic 체크리스트로 폴백합니다.
...
- **머지 후 정리**: `/css:clean`이 dirty·미푸시·미머지 안전 점검과 함께 worktree/로컬 브랜치를 정리
