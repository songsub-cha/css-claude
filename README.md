...
설치 후, 두 환경 모두 **동일한 파이프라인**을 사용합니다 — 차이는 커맨드 네임스페이스(`css:` ↔ `css-`)뿐입니다.
...
먼저 설치: `bash scripts/install-codex.sh` (Windows: `scripts\install-codex.ps1`). Codex는 `:` 네임스페이스가 없어 **`css-` 프리픽스**를 씁니다:
...
전체 커맨드 레퍼런스는 [`docs/usage.ko.md`](docs/usage.ko.md), Codex 설치·사용은 [`docs/installation.ko.md`](docs/installation.ko.md)의 Codex CLI 섹션을 참고하세요.
