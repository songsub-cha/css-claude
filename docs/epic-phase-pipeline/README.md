...
| 수준 | plan | review |
|------|------|--------|
| **Epic** (`kind=epic`) | 스켈레톤: 코드 없는 거친 태스크 타이틀 + 배치 그룹 | 아키텍처 review: 커버리지 매트릭스 + Phase 열 + 전문가 라우팅. **rich-spec 없음** |
...
```json
{
  ...
  "phases": {
    ...
    "review":   {"level": "rich-spec", "verdict": "PASS"},
